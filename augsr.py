...
def get_resize(size: int):
    pipeline = albu.Compose([albu.Resize(size//2, size//2)], additional_targets={'target': 'image'}) #600,600
    def process(a, b):
...
def get_normalize():
    normalize = albu.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]) #albu.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5],max_pixel_value=2**16-1)
    normalize = albu.Compose([normalize], additional_targets={'target': 'image'}) #albu.Normalize(mean=[0.0], std=[1.0]) #
