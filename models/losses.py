...
class ContentLoss():
    ...
    def get_loss(self, fakeIm, realIm):
        ...
        if isinstance(fakeIm,list):
            return self.criterion(fakeIm[0], realIm[0]) + self.criterion(fakeIm[1], realIm[1])
        else:
            return self.criterion(fakeIm, realIm) + TVLoss(TVLoss_weight=0.01)(fakeIm)

    def __call__(self, fakeIm, realIm):
        return self.get_loss(fakeIm, realIm)

class ContentCenterLoss():
    def initialize(self, loss):
        self.criterion = loss

    def get_loss(self, fakeIm, realIm):
        return self.criterion(fakeIm[:,:,100:-100,100:-100], realIm[:,:,100:-100,100:-100])
...
def get_loss(model):
    ...
    elif model['content_loss'] == 'ssim':
        content_loss = ContentLoss()
        content_loss.initialize(SSIM()) #ssim = SSIM().type(dtype)
    elif model['content_loss'] == 'l2':
        content_loss = ContentLoss()
        content_loss.initialize(nn.MSELoss())
    elif model['content_loss'] == 'l2c':
        content_loss = ContentCenterLoss()
        content_loss.initialize(nn.MSELoss())
    else:
