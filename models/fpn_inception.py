...
class FPNInception(nn.Module):
    ...
    def forward(self, x):
        ...
        map4 = nn.functional.upsample(self.head4(map4), scale_factor=8, mode="bilinear") #nearest
        map3 = nn.functional.upsample(self.head3(map3), scale_factor=4, mode="bilinear")
        map2 = nn.functional.upsample(self.head2(map2), scale_factor=2, mode="bilinear")
        map1 = nn.functional.upsample(self.head1(map1), scale_factor=1, mode="bilinear")
...
class FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        pad1 = (0, 1, 0, 1)
        map4 = lateral4
        map3 = self.td1(lateral3 + nn.functional.upsample(map4, scale_factor=2, mode="bilinear")) # bilinear , nearest
        map2 = self.td2(F.pad(lateral2, pad, "reflect") + nn.functional.upsample(map3, scale_factor=2, mode="bilinear"))
        map1 = self.td3(lateral1 + nn.functional.upsample(map2, scale_factor=2, mode="bilinear"))
